...
test_that("checkmate assertions", {
  ...
  expect_error(generate_motif(alphabet, n, d, c("w", "e", "i", "g", "h", "t", "s")))
...
test_that("generate list of motifs", {
  ...
  motifs <- generate_motifs(alphabet, n_motifs, n, d, weights)
  ...
  motifs <- generate_motifs(alphabet, 1, n, d, weights, validate = FALSE)
...
test_that("motif injection", {
  ...
  motifs <- generate_motifs(letters[1:4], 2, 4, 2)
  injected_sequence <- add_motifs(motifs, sequence)
  expect_equal(injected_sequence,
               structure(c("a", "c", "b", "d", "c", "c", "d", "d"),
                         motifs = list(c("d", "c"),
                                       c("a", "c", "_", "_", "c")),
                         masks = list(c(FALSE, FALSE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE),
                                      c(TRUE, TRUE, FALSE, FALSE, TRUE, FALSE, FALSE, FALSE))))
...
test_that("set of motifs cannot be injected", {
  ...
  expect_error(add_motifs(generate_motifs(letters[1:4], 5, 3, 1),
                          sequence))
